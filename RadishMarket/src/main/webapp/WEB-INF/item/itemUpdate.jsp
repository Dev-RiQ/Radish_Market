...
<div class="item-insert-box ">
	...
	<form action="/updateItem.do" method="post" enctype="multipart/form-data">
		...
		<div class="txt" style="margin-bottom: 10px"><h3>상품 이미지</h3></div>
			...
			<div class="post-list" id="post-list">
	          ...
	          <div class="imgbtn">
	            <button type="button" onclick="fileUpload()">
	              <!--이미지 업로드 버튼-->
	              <i class="fa-solid fa-image" style="font-size: 25px; margin-top: 7px"></i><br />
	              <p>이미지</p>
	            </button>
	          </div>
        </div>
        ...
        <select class="category" id="item_category_no" name="item_category_no">
        ...
        <input type="number" id="item_price" name="item_price" value="${item.item_price}" placeholder="₩ 가격을 입력해주세요.">
		<span id="price_check"></span>
		 
        <div class="txt">
          <h3>판매 상태<span> *</span></h3>
        </div>
		<select class="category" id="item_status" name="item_status">
			<option value="1" ${ item.item_status == 1 ? 'selected' : '' }>판매 중</option>
			<option value="2" ${ item.item_status == 2 ? 'selected' : '' }>예약 중</option>
			<option value="3" ${ item.item_status == 3 ? 'selected' : '' }>판매 완료</option>
		</select> 

        <div class="registrationbtn">
          <button onclick="history.back()">취소</button><!--취소-->
          <button onclick="location.href='deleteItem.do?item_no=${item.item_no}'">삭제하기</button><!--등록-->
          <button class="rbtn">수정하기</button><!--등록-->
        </div>
	</form>
	</div>
	</div>
