								...
								<c:otherwise>
									<c:forEach var="i" begin="0" end="${itemImgList.size()-1}">
										<div class="swiper-slide">
											<img class="load-img" alt="item-images"
												src="/images/${itemImgList.get(i)}">
										</div>
									</c:forEach>
								</c:otherwise>
							</c:choose>
						...
						<c:choose>
							<c:when test="${item.user_no ne log}">
								<input type="hidden" name="user_no" id="user_no"
									value="${ item.user_no }">
								<input type="hidden" name="alarm_category_no"
									id="alarm_category_no" value="3">
								<input type="hidden" name="link_no" id="link_no"
									value="${ item.item_no }">
								<input type="hidden" name="isZzim" id="isZzim"
									value="${ isZzim ne null ? isZzim : 0}">
								<c:choose>
				...
				<c:forEach var="i" begin="0" end="${hotItemList.size()-1}">
					<div class="child" style="cursor: pointer;"
						...
						<div class="saleprice">
							...
							<h5>${hotItemPriceList.get(i)}원</h5>
							<span>${hotUserNicknameList.get(i)}</span> <span> <c:choose>
