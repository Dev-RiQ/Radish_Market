			...
			<div class="thelook">
				...
				<div class="itemandtxt">
					<div class="itemimg">
						...
						<div class="txts">
							<div class="propir">
								<img class="propir-img" alt="유저 이미지"
									src="/images/${user.user_img ne '' ? user.user_img : 'usersDefaultImg.png'}">
								<div class="namebox">
									<p><a href='/itemListUser.do?user_no=${user.user_no}'>${user.user_nickname}</a></p>
									<span><a
											href='/listItem.do?filter=true&gu=${user.user_gu}&dong=${user.user_dong}'>${user.user_dong}</a></span>
								</div>
							</div>
							<div class="ondobox">
								<!--온도-->
							...
							<div class="btn-box">
								<c:choose>
									<c:when test="${item.user_no ne log}">
										<input type="hidden" name="user_no" id="user_no" value="${ item.user_no }"> <input type="hidden"
											name="alarm_category_no" id="alarm_category_no" value="3">
										<input type="hidden" name="link_no" id="link_no" value="${ item.item_no }"> <input type="hidden"
								...
								<c:forEach var="i" begin="0" end="${itemList.size()-1}">
									<div class="child" style="cursor: pointer;"
										...
										<div class="saleprice">
											...
											<h5>
												${itemPriceList.get(i)}원
											</h5>
											<span>${user.user_dong}</span>
											<span>
												<c:choose>
