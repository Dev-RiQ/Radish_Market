...
<div class="board-info-box">
        <div class="lifethelook">
           <div class="titlebox"><a href='/index.jsp'>홈 > </a> <a href='/listBoard.do'>동네생활 > </a> <span>${board.board_title}</span></div>
            ...
            <div class="lifethelooktxt">
				...
				<button>필터적용</button>
                  ...
                  <div class="pepoletxts">
                    ...
                    <div class="propil">
                        <table>
                           <tr><td><button onclick="location.href='/listBoard.do?filter=true&category_no=${board.board_category_no}'">동네친구</button></td><td><button onclick="location.href='/listBoard.do'">목록</button></td>
<c:if test="${ board.user_no == log }">
	<td><button onclick="location.href='/deleteBoard.do?board_no=${ board.board_no }'">삭제하기</button></td>
	<td><button onclick="location.href='/updateBoard.do?board_no=${ board.board_no }'">수정하기</button></td>
</c:if> <!--게시글 유형--></tr>
                           
                                <tr><td rowspan="2"> <div id="propilimg"><img alt="이미지" src="/images/${ user.user_img ne '' ? user.user_img : 'usersDefaultImg.png' }" /></div></td> <!--프로필-->
                               
                                <td><a href="/userItemList.do?user_no=${ user.user_no }">${ user.user_nickname }</a></td> <!--이름-->
                                <td>${ user.user_deg }℃</td></tr> <!--온도-->
                                <tr><td colspan="5"><span><a href="listBoard.do?filter=true&dong=${user.user_dong }">${ user.user_dong }</a></span></td></tr> <!--주소,시간-->
                        ...
                        <div class="propiltxt">
                            ...
                            <div class="likeandtxtbox">
                              <div class="iconbox">
                                ...
                                <button id="btn-like-submit" type="button" onclick="sendAlarm()"><i class="fa-solid fa-heart" style="color: ${isLike ne null && isLike == 1 ? 'red':'#ddd'};"></i> <span id="like-count">${ likeCount }</span></button> <!--좋아요-->
                            ...
                            <div class="comments">
                            	<c:if test="${commentList.size() != 0}">
									<c:forEach var="i" begin="0" end="${commentList.size() - 1}">
			                              <table>
			                              	<tr>
			                                <td rowspan="2" style="width: 40%;" > <div id="propilimg"><img alt="이미지" src="/images/usersDefaultImg.png"/></div></td> <!--댓글 프로필-->
			                                <td>${ commentNickname.get(i) }</td> <!--이름-->
			                                </tr>
			                                <tr><td colspan="5"><span>${ commentList.get(i).comment_reg_datetime } ${ commentList.get(i).check_update == 1 ? '수정됨' : '' }</span></td></tr> <!--온도-->
			                              </table>
                      ...
                      <div class="writingout">
						...
						<button id="btn-comment-submit" onclick="sendAlarm()"><i class="fa-solid fa-paper-plane"></i></button>
                      </div>            
            </div>
            </div>
            </div>
