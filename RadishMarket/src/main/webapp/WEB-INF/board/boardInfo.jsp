				...
				<div class="besttexts">
					<form action="/listBoard.do?filter=true" method="post">
						...
						<c:forEach var="category_dong" items="${ dongList }">
							<label class="container">
								<input type="radio" name="dong" id="dong" value="${ category_dong }"
								${user_dong eq category_dong ? 'checked' : '' } />
								${ category_dong } 
								<span class="checkmark"></span>
							</label>
						</c:forEach>
						...
						<br> 
						<label class="container">
							<input type="radio" name="category_no" id="category_no" value="0" 
							${category_no eq null || category_no == 0 ? 'checked' : ''} />전체보기
							<span class="checkmark"></span> 
						</label>
						<c:forEach var="category" items="${ categoryList }">
							<label class="container">
								<input type="radio" name="category_no" id="category_no"
									value="${ category.board_category_no }" ${ category_no eq category.board_category_no ? 'checked': '' } />
						...
						<label class="container">
							<input type="radio" name="order_by" id="order_by" value=0
						...
						<table>
							<tr>
								...
								<c:if test="${ board.user_no == log }">
									<td><button
											onclick="location.href='/deleteBoard.do?board_no=${ board.board_no }'">삭제하기</button>
									</td>
									<td><button
											onclick="location.href='/updateBoard.do?board_no=${ board.board_no }'">수정하기</button>
									</td>
								</c:if>
								<!--게시글 유형-->
							</tr>
							...
							<tr>
								<td rowspan="2">
									<div id="propilimg">
										<img alt="이미지"
											src="/images/${ user.user_img ne '' ? user.user_img : 'usersDefaultImg.png' }" />
									</div>
								</td>
								<!--프로필-->
							...
							<!--온도-->
							<td><progress id="progress" value="${user.user_deg}"
									max="100"></progress></td>
							<tr>
								<td colspan="5">
									<span>
							...
							<div class="likeandtxtbox">
								<div class="iconbox">
									<input type="hidden" name="user_no" id="user_no"
										value="${ board.user_no }"> <input type="hidden"
									...
									<input type="hidden" name="link_no" id="link_no"
										...
										name="isLike" id="isLike"
										value="${ isLike ne null ? isLike : 0 }">
									<button id="btn-like-submit" type="button"
										onclick="sendAlarm()">
								...
								<c:if test="${commentList.size() != 0}">
									<c:forEach var="i" begin="0" end="${commentList.size() - 1}">
										...
										<div class="comments-txt"
											id="comment_content_box${ commentList.get(i).comment_no }">
											...
											<c:if test="${ log == commentList.get(i).user_no }">
												<button
													id="btn-comment-update${ commentList.get(i).comment_no }"
													onclick="commentUpdate(this)">수정</button>
											</c:if>
											<c:if
												test="${ log == commentList.get(i).user_no || log == board.user_no }">
												<button
