...
public class UpdateCalendarController implements Controller {
	...
	public String requestHandler(HttpServletRequest request, HttpServletResponse response)
		...
		Calendar calendar = new Calendar(main_user_no, address, calendar_dir_x, calendar_dir_y, calendar_datetime, calendar_title, calendar_content);
		if(CalendarDAO.getInstance().insertACalenadr(calendar))
			AlertUtil.getInstance().goHomeWithAlert(response, "일정 수정 완료");
		else
